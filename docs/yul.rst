...
标有 ``F``， ``H``， ``B``， ``C``， ``I``， ``L``， ``P`` 和 ``N`` 的操作码分别
是从Frontier，Homestead，Byzantium，Constantinople，Istanbul，London，Paris或Cancun版本出现的。

在下文中， ``mem[a...b)`` 表示从位置 ``a`` 开始到不包括位置 ``b`` 的内存字节，
``storage[p]`` 表示插槽 ``p`` 的存储内容。
同样地， ``transientStorage[p]`` 表示插槽 ``p`` 处的临时存储内容。
...
+-------------------------+------+-----+------------------------------------------------------------------+
| sdiv(x, y)              |      | F   | x / y，对于有符号的二进制补码，如果 y == 0，则为 0               |
+-------------------------+------+-----+------------------------------------------------------------------+
| mod(x, y)               |      | F   | x % y, 如果 y == 0，则为 0                                       |
+-------------------------+------+-----+------------------------------------------------------------------+
| smod(x, y)              |      | F   | x % y, 对于有符号的二进制补码, 如果 y == 0，则为 0               |
+-------------------------+------+-----+------------------------------------------------------------------+
| exp(x, y)               |      | F   | x的y次方                                                         |
+-------------------------+------+-----+------------------------------------------------------------------+
...
+-------------------------+------+-----+------------------------------------------------------------------+
| slt(x, y)               |      | F   | 如果 x < y，则为1，否则为0，适用于有符号的二进制补码             |
+-------------------------+------+-----+------------------------------------------------------------------+
| sgt(x, y)               |      | F   | 如果 x > y，则为1，否则为0，适用于有符号的二进制补码             |
+-------------------------+------+-----+------------------------------------------------------------------+
...
+-------------------------+------+-----+------------------------------------------------------------------+
| calldatacopy(t, f, s)   | `-`  | F   | 从位置f的调用数据复制s个字节到位置t的内存中                        |
+-------------------------+------+-----+------------------------------------------------------------------+
| codesize()              |      | F   | 当前合约/执行环境的代码大小                                      |
+-------------------------+------+-----+------------------------------------------------------------------+
| codecopy(t, f, s)       | `-`  | F   | 从位置f的code中复制s个字节到位置t的内存中                          |
+-------------------------+------+-----+------------------------------------------------------------------+
| extcodesize(a)          |      | F   | 地址为a的代码的大小                                              |
+-------------------------+------+-----+------------------------------------------------------------------+
| extcodecopy(a, t, f, s) | `-`  | F   | 像 codecopy(t, f, s) 一样，但在地址a处取代码                     |
+-------------------------+------+-----+------------------------------------------------------------------+
| returndatasize()        |      | B   | 最后返回数据的大小                                               |
+-------------------------+------+-----+------------------------------------------------------------------+
| returndatacopy(t, f, s) | `-`  | B   | 从位置f的returndata复制s个字节到位置t的内存中                      |
+-------------------------+------+-----+------------------------------------------------------------------+
| mcopy(t, f, s)          | `-`  | N   | 从位置f的内存复制s个字节到位置t的内存中         |
+-------------------------+------+-----+------------------------------------------------------------------+
| extcodehash(a)          |      | C   | 地址a的代码哈希值   |
+-------------------------+------+-----+------------------------------------------------------------------+
|| create(v, p, n)        ||     || F  || 创建新合约，其代码为内存中位置p到(p+n)的内容，发送v数量的wei     |
||                        ||     ||    || 并返回新地址；错误时返回0                  |
+-------------------------+------+-----+------------------------------------------------------------------+
|| create2(v, p, n, s)    ||     || C  || 创建新合约，其代码为内存中位置p到(p+n)的内容         |
||                        ||     ||    || 合约地址为 keccak256(0xff . this . s . keccak256(mem[p...(p+n)))|
||                        ||     ||    || 并发送v数量wei然后返回新地址， 其中 ``0xff`` 是一个1字节的值，  |
||                        ||     ||    || ``this`` 是当前合约的地址，        |
||                        ||     ||    || 是一个20字节的值，  ``s`` 是一个大端序的256位值；     |
||                        ||     ||    || 错误时返回0            |
+-------------------------+------+-----+------------------------------------------------------------------+
|| call(g, a, v, in,      ||     || F  || 调用地址为a的合约，输入数据为内存中位置in到(in+insize)的内容     |
|| insize, out, outsize)  ||     ||    || 一并发送g数量的燃料和v数量的wei，并将输出写入内存中              |
||                        ||     ||    || 位置out到(out+outsize)的区域。 如果错误，返回0（比如，燃料耗尽）  |
||                        ||     ||    || 成功则返回1                                                |
||                        ||     ||    || :ref:`查看更多 <yul-call-return-area>`    |
+-------------------------+------+-----+------------------------------------------------------------------+
|| callcode(g, a, v, in,  ||     || F  || 相当于 ``call`` 但仅仅使用地址a上的代码     |
...
||                        ||     ||    || :ref:`查看更多 <yul-call-return-area>`     |
+-------------------------+------+-----+------------------------------------------------------------------+
| return(p, s)            | `-`  | F   | 终止执行，并返回内存中位置p到(p+s)的数据                       |
+-------------------------+------+-----+------------------------------------------------------------------+
| revert(p, s)            | `-`  | B   | 终止执行，回滚状态更改，并返回内存中位置p到(p+s)的数据  |
+-------------------------+------+-----+------------------------------------------------------------------+
|| selfdestruct(a)        || `-` || F  || 终止执行，销毁当前合约，并且将余额发送到地址a     |
||                        ||     ||    || （已废弃）  |
+-------------------------+------+-----+------------------------------------------------------------------+
| invalid()               | `-`  | F   | 以无效指令终止执行                           |
+-------------------------+------+-----+------------------------------------------------------------------+
| log0(p, s)              | `-`  | F   | 用内存中位置p到(p+s)的内容产生日志            |
+-------------------------+------+-----+------------------------------------------------------------------+
| log1(p, s, t1)          | `-`  | F   | 用内存中位置p到(p+s)的内容和 topic t1 产生日志               |
+-------------------------+------+-----+------------------------------------------------------------------+
| log2(p, s, t1, t2)      | `-`  | F   | 用内存中位置p到(p+s)的内容和 topic t1，t2 产生日志            |
+-------------------------+------+-----+------------------------------------------------------------------+
| log3(p, s, t1, t2, t3)  | `-`  | F   | 用内存中位置p到(p+s)的内容和 topic t1，t2，t3 产生日志           |
+-------------------------+------+-----+------------------------------------------------------------------+
|| log4(p, s, t1, t2, t3, || `-` || F  || 用内存中位置p到(p+s)的内容和 topic t1，t2，t3，t4 产生日志       |
|| t4)                    ||     ||    ||   |
+-------------------------+------+-----+------------------------------------------------------------------+
...
+-------------------------+------+-----+------------------------------------------------------------------+
| blobbasefee()           |      | N   | 当前区块的blob的基本费用 （EIP-7516和EIP-4844）            |
+-------------------------+------+-----+------------------------------------------------------------------+
| origin()                |      | F   | 交易发送者                                               |
+-------------------------+------+-----+------------------------------------------------------------------+
| gasprice()              |      | F   | 交易的燃料价格                                     |
...
+-------------------------+------+-----+------------------------------------------------------------------+
| blobhash(i)             |      | N   | 交易的第i个blob的版本化哈希值                        |
+-------------------------+------+-----+------------------------------------------------------------------+
| coinbase()              |      | F   | 当前的挖矿的受益者                                       |
+-------------------------+------+-----+------------------------------------------------------------------+
| timestamp()             |      | F   | 自epoch开始的，当前块的时间戳，以秒为单位        |
+-------------------------+------+-----+------------------------------------------------------------------+
| number()                |      | F   | 当前区块号                                             |
+-------------------------+------+-----+------------------------------------------------------------------+
