...
Solidity编译器涉及三个不同级别的优化（按执行顺序）： 

- 基于对Solidity代码的直接分析，在代码生成过程中进行优化。
- 优化Yul IR代码的转换。
- 操作码级别的优化。
...
基于代码生成器的优化器会影响从Solidity输入生成的初始低级代码。
在旧的优化流程中，字节码是立即生成的，而这种优化大多是隐式的且不可配置的，
唯一的例外是改变二进制操作中字面量顺序的优化。
基于IR的优化流程采用了不同的方法，它生成的Yul IR与Solidity代码的结构非常匹配，
几乎所有的优化都交由Yul优化模块完成。
在这种情况下，仅有很少几种难以在Yul IR中处理，但在分析阶段的高级信息下就可以直接处理，
才进行代码生成级别的优化。
这种优化的一个例子是在某些惯用的 ``for`` 循环中，递增计数器时绕过校验运算。
...
.. note::
    `窥孔优化器（peephole optimizer） <https://en.wikipedia.org/wiki/Peephole_optimization>`_
    和 :ref:`未检查循环增量优化器 <unchecked-loop-optimizer>` 在默认情况下总是启用的，
...
目前，基于代码生成器的优化模块提供两种优化方式。

第一个在传统代码生成器中可用，它将字面移动到交换二元运算符的右侧，这有助于利用它们的关联性。

另一个在基于IR的代码生成器中可用，允许在为特定惯用的 ``for`` 循环的计数变量生成代码时使用未经检查的算术，
这样可以避免浪费燃料，因为它识别出一些条件，确保计数变量不会溢出。
这样就无需在循环体内部使用冗长的unchecked算术块来递增计数变量。
...
溢出检查优化步骤是在 Solidity ``0.8.22`` 中引入的，
主要是确定在哪些条件下可以安全地递增 ``for`` 循环计数器而无需进行溢出检查。

这种优化 **只** 适用于一般形式的 ``for`` 循环：
...
该条件和计数器变量只递增的事实保证了计数器变量永远不会溢出。
符合循环优化条件的确切要求如下：

- 循环条件是局部计数变量 ``i`` （此处称为“循环计数器”）和表达式 ``Y`` 的比较形式为 ``i < Y``。
- 内置运算符 ``<`` 必须在循环条件中使用，并且它是唯一触发优化的运算符。
  ``<=`` 等运算符被有意排除在外。此外，用户定义的运算符也 **不** 符合条件。
- 循环表达式是计数变量的前缀或后缀增量，即 ``i++`` 或 ``++i``.
- 循环计数器是一个内置整数类型的局部变量。
- 循环计数器 **不会** 被循环体或作为循环条件的表达式修改。
- 比较是在与循环计数器相同的类型上进行的，这意味着右侧表达式的类型可以隐式转换为计数器的类型，
  使得后者在比较之前不会被隐式扩展。
...
在这种情况下，计数器 ``i`` 的类型在比较之前已从 ``uint8`` 隐式转换为 ``uint16`` ，
虽然条件实际上永远不会为假，但是不能删除增量的溢出检查。
