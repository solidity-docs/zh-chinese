...
当调用其他合约的函数时，您可以用特殊的选项 ``{value: 10, gas: 10000}`` 指定随调用发送的wei或燃料（gas）数量。
请注意，不鼓励明确指定燃料值，因为操作码的燃料成本可能在未来发生变化。
...
.. note::
    在 Solidity 0.6.2 之前，指定以太值和燃料值的推荐方法是
...
表达式的计算顺序不是特定的（更准确地说，
表达式树中某节点的子节点间的计算顺序不是特定的，但它们的结算肯定会在节点自己的结算之前）。
...
.. note::
    ...
    它消耗了所有可用于调用的燃料。在Metropolis发布之前，
    使用 ``require`` 的异常会消耗所有燃料。
