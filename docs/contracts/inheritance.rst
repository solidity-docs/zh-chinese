...
参见 :ref:`函数重写 <function-overriding>` 以了解更多细节。
...
<<<<<<< HEAD
        ...
        // 如果您想重写这个函数，您需要使用 `override` 关键字。
        // 如果您想让这个函数再次被重写，您需要再指定 `virtual` 关键字。
...
<<<<<<< HEAD
调用 ``Final.destroy()`` 时会调用最后的派生重写函数 ``Base2.destroy``，
...
被重写的函数必须在函数头中使用 ``override`` 关键字。
重写函数只能将被重写函数的可见性从 ``external`` 改为 ``public``。
...
并且还没有被另一个基类合约重写（在继承图的某个路径上）。
此外，如果一个合约从多个（不相关的）基类合约上继承了同一个函数，必须明确地重写它。
...
如果函数被定义在一个共同的基类合约中，
或者在一个共同的基类合约中有一个独特的函数已经重写了所有其他的函数，
则不需要明确的函数重写指定符。
...
.. note::
  ...
  从Solidity 0.8.8开始，当重写一个接口函数时，
  不需要 ``override`` 关键字，除非该函数被定义在多个基础上。


如果函数的参数和返回类型与变量的getter函数匹配，公共状态变量可以重写为外部函数。
...
函数修改器可以相互重写。
这与 :ref:`函数重写 <function-overriding>` 的工作方式相同（除了对修改器没有重写）。
``virtual`` 关键字必须用在被重写的修改器上， ``override`` 关键字必须用在重写的修改器上：
...
由于您必须明确地重写一个从多个基类合约继承的函数，
而没有唯一的重写，C3线性化在实践中不是太重要。
